...
export class CredixMasService {
  ...
  _subscribeResponse: {type: 'success' | 'error', message: string, title: string};
  ...
  get response(): {title: string, message: string, type: 'success' | 'error'} {
  ...
  getBenefits(): Tag[] {
    return this._tags.filter(el => el.description.includes("credixmas.benefits"));
  }

  getAccountInfo(){
    return this.http.post('canales', this.URL_ACCOUNT_INFO, {
  ...
  subscribe(){
    return this.http.post('canales', this.URL_SUBSCRIBE, {
      accountId: this.storage.getCurrentUser().actId
    }).pipe(
      map(response => {
