...
export class ExtendTermService {
  ...
  private $allowedMovementsState: Subject<AllowedMovement[]> = new Subject<
    AllowedMovement[]
  >();
  private $filterPromo: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(
    false
  );
  private $hideButtonPromoFilter: BehaviorSubject<boolean> =
    new BehaviorSubject<boolean>(false);
  private $disableCheckBoxState: BehaviorSubject<boolean> =
    new BehaviorSubject<boolean>(false);
  ...
  getAllowedMovements(productId: number): Observable<any> {
    return this.httpService
      .post("canales", this.getAllowedMovementsUri, {
        accountId: this.storageService.getCurrentUser().actId,
        cardId: this.storageService
          .getCurrentCards()
