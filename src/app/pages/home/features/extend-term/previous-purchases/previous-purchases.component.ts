...
export class PreviousPurchasesComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.landingService
      .getHomeContent(this.cardId)
      .subscribe(
        (response) =>
          (this.endDate = ConvertStringDateToDate(
            response.fechaProximaMaximaPago
          ))
      );
  ...
  calculateTotalAmountSelect(
    ...
    if (checked) {
      ...
      if (
        this.amountArray.findIndex((obj) => obj.movementId === movementId) ===
        -1
      ) {
      ...
      this.amountArray.splice(
        this.amountArray.findIndex((obj) => obj.movementId === movementId),
        1
      );
    }
    totalAmount = this.amountArray.reduce(
  ...
  change(checked: boolean, movement: PreviousMovements, i: number) {
    ...
    if (checked) {
        ...
        return {
          ...
          productDisable:
            ((movement.quota === 0 || movement.quota === 1) &&
              obj.quota > movement.quota &&
              obj.quota !== 1) ||
            this.previousMovementsSelected.findIndex(
              (value) =>
                value.originCurrency.currencyId ===
                obj.originCurrency.currencyId
            ) < 0,
      ...
      this.selection.splice(
        this.selection.findIndex((mov) => mov === movement.pdqId),
        1
      );
      this.checkedListStates[i] = false;
      this.previousMovementsSelected.splice(
        this.previousMovementsSelected.findIndex(
          (elem) => elem.pdqId === movement.pdqId
        ),
        1
      );
      this.previousMovements = this.previousMovements.map((obj, index) => {
    ...
    this.extendTermService.movementsSelectedArr =
