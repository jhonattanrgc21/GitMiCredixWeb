...
export class LandingComponent implements OnInit, OnDestroy {
  ...
  getHomeContent() {
    this.landingService.getHomeContent(this.cardId).subscribe((response) => {
      if (response) {
        ...
        this.modalService.showModalInfo("credixmas").subscribe((value) => {
          if (value) {
  ...
  getAccountsSummary() {
    this.channelsApiService
      .getAccountSummary(this.cardId)
      .subscribe((accountSummary) => (this.accountSummary = accountSummary));
  }
  ...
  getHomeTags() {
    this.landingService.getHomeTags().subscribe((response) => {
      if (response.type === "success" && response.tagsHome?.tags.length > 0) {
        this.balancesTags = {
          personalCreditAvailable: response.tagsHome.tags.find(
            (tag) =>
              tag.description === "disponible.tag.disponiblecreditopersonal"
          ).value,
          linkapplyforcredit: response.tagsHome.tags.find(
            (tag) => tag.description === "disponible.tag.solicitarcredito"
          ).value,
          creditLimitTag: response.tagsHome.tags.find(
            (tag) => tag.description === "disponible.tag.limitecredito"
          ).value,
          consumedTag: response.tagsHome.tags.find(
            (tag) => tag.description === "disponible.tag.consumido"
          ).value,
          availableTag: response.tagsHome.tags.find(
            (tag) => tag.description === "disponible.tag.disponible"
          ).value,
          increaseCreditLimitTag: response.tagsHome.tags.find(
            (tag) => tag.description === "disponible.link.aumentarlimitecredito"
          ).value,
          ibanAccountsTag: response.tagsHome.tags.find(
            (tag) => tag.description === "pago.link.cuentasiban"
          ).value,
          colonesTag: response.tagsHome.tags.find(
            (tag) => tag.description === "pago.table.header.colones"
          ).value,
          dollarsTag: response.tagsHome.tags.find(
            (tag) => tag.description === "pago.table.header.dolares"
          ).value,
          colonesIbanCopiedTag: response.tagsHome.tags.find(
            (tag) => tag.description === "pago.popup.message.success.copy"
          ).value,
          dollarsIbanCopiedTag: "Cuenta IBAN dólares copiada",
        };

        this.paymentDetailsTags = {
          titleTag: response.tagsHome.tags.find(
            (tag) => tag.description === "pago.table.header.detallepago"
          ).value,
          cutOffTag: response.tagsHome.tags.find(
            (tag) => tag.description === "pago.barra.corte"
          ).value,
          maxPaymentDateTag: response.tagsHome.tags.find(
            (tag) => tag.description === "pago.barra.maximapago"
          ).value,
          colonesTag: response.tagsHome.tags.find(
            (tag) => tag.description === "pago.popup.tag.colones"
          ).value,
          dollarsTag: response.tagsHome.tags.find(
            (tag) => tag.description === "pago.popup.tag.dolares"
          ).value,
          minTag: response.tagsHome.tags.find(
            (tag) => tag.description === "pago.table.row.minimo"
          ).value,
          countedTag: response.tagsHome.tags.find(
            (tag) => tag.description === "pago.table.row.contado"
          ).value,
          todayTag: response.tagsHome.tags.find(
            (tag) => tag.description === "pago.barra.hoy"
          ).value,
        };

        this.movementsTags = {
          titleTag: response.tagsHome.tags.find(
            (tag) => tag.description === "movimientos.title.movimientos"
          ).value,
        };
      }
    });
  ...
  openModalCredixMas(credixMasTitle: string, credixMasText: string) {
    ...
    credixMasText =
      "Con Credix Más podrá: Extender el plazo de pago de sus compras de manera gratuita e ilimitada. Compras a 6 cuotas cero interés en tiendas seleccionadas.";
