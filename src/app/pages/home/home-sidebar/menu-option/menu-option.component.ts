...
export class MenuOptionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.routeChanged();
    this.homeNavigationMenuService.closeSubmenuObs.subscribe(
  ...
  routeChanged() {
    this.navigationService.submenuChanged$.subscribe((route) => {
      const submenu = this.submenus.find(
        (sub) => sub.route.split("/")[2] === route
      );
      const menu = this.menus.find((men) => men.name === submenu.parentId);
      this.menuChanged(
        menu.id,
        menu.route,
        this.submenus.filter((sub) => sub.parentId === menu.name).length,
        false
      );
  ...
  subscribeToTags() {
    this.tagsService
      ...
      .subscribe((functionality) => {
        if (this.enableIncreaseCreditLimit)
          this.getTags(
            functionality.find(
              (fun) => fun.description === "Aumentar límite de crédito"
            ).tags
          );
  ...
  getMenus() {
    this.tagsService
      ...
      .subscribe((functionalities) => {
        this.enableIncreaseCreditLimit =
          this.tagsService.enableIncreaseCreditLimit;
        this.isSchedulaExtendTermAvailable =
          this.tagsService.isSchedulaExtendTermAvailable;
        if (functionalities.length > 0) {
          this.submenus = [];
          functionalities.forEach((func) => {
            if (func.level === 2) {
              if (
                func.link == "/home/increase-limit" &&
                !this.enableIncreaseCreditLimit
              )
                return;
              ...
              if (func.link == "/home/personal-credit") {
                this.homeNavigationMenuService.setEnableOptionPersonalCredit(
                  true
                );
              }
              if (
                func.link == "/home/schedule-quotas" &&
                !this.isSchedulaExtendTermAvailable
              )
                return;
        ...
        this.menus.forEach((menu) => {
          if (menu.id !== 1) {
            menu.submenus = this.submenus.filter(
              (sub) => sub.parentId === menu.name
            );
          }
        });
  ...
  menuChanged(
    ...
    this.openSubmenu =
      this.preActiveMenu === menuId ? !this.openSubmenu : !isFirst;
    ...
    if (this.activeSubmenu !== submenuId) {
      ...
      if (route === "/home/increase-limit") {
        this.modalService
          .confirmationPopup(
            this.questionTag ||
              "¿Desea solicitar el aumento de límite de crédito?"
          )
  ...
  getTags(tags: Tag[]) {
    this.questionTag = tags.find(
      (tag) => tag.description === "aumento.question"
    ).value;
  }
        ...
        case "Pagar":
          if (menu.submenus) {
            ...
            payOrdering.forEach((orderFlag) => {
              const found = menu.submenus.find(
                (submenu) => submenu.name === orderFlag
              );
              if (found) {
...
export const submenus = [
  {
    id: 1,
    name: "Servicios",
    route: "/home/public-services",
    icon: "public_services",
    parentId: "Pagar",
  },
  {
    id: 2,
  ...
  },
  {
    id: 4,
    name: "Enviar dinero",
    route: "/home/send-money",
    icon: "transfer",
    parentId: "Pagar",
  },
  {
    id: 5,
    name: "Reportar transferencia",
    route: "/home/report-transference",
    icon: "transfer_report",
    parentId: "Pagar",
  },
  {
    id: 6,
    name: "Lugares de pago",
    route: "/home/payment-places",
    icon: "map-marker",
    parentId: "Pagar",
  },
  {
    ...
    route: "/home/personal-credit",
    icon: "personal_credit",
    parentId: "Productos",
  },
  {
    id: 8,
    name: "Compra sin tarjeta",
    route: "/home/buy-without-card",
    icon: "code",
    parentId: "Productos",
  },
  {
    id: 9,
    name: "Ampliar plazo de compra",
    route: "/home/extend-term",
    icon: "anticipated_canc",
    parentId: "Productos",
  },
  {
    id: 10,
    name: "Plan liquidez",
    route: "/home/extend-term-total-debt",
    icon: "anticipated_canc",
    parentId: "Productos",
  },
  {
    ...
    route: "/home/anticipated-cancellation",
    icon: "anticipated_canc",
    parentId: "Productos",
  },
  {
  ...
  },
  {
    ...
    route: "/home/favorites-management",
    icon: "favorites",
    parentId: "Mi Cuenta",
  },
  {
  ...
  },
  {
    id: 15,
    name: "Cambiar PIN",
    route: "/home/change-pin",
    icon: "asterisk",
    parentId: "Mi Cuenta",
  },
  {
    id: 16,
    name: "Aumentar límite de crédito",
  ...
  {
    id: 18,
    name: "Programar cuotas",
    route: "/home/schedule-quotas",
    icon: "calendar",
    parentId: "Productos",
  },
  {
    id: 19,
    name: "Suscripción Credix Más",
    route: "/home/credix-mas",
    icon: "tabs",
    parentId: "Mi Cuenta",
  },
];
