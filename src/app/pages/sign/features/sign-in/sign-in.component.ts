  ...
  open(modal: 'sign-up' | 'forgot-pass' | 'session-activate' | 'new-device') {
    switch (modal) {
      ...
      case 'session-activate':
        ...
        break;
      case 'new-device':
        this.newDeviceModal = this.modalService.open({template: this.newDeviceTemplate, hideCloseButton: false, title: 'Validación de Identidad'},
    ...
    this.httpService.post('canales','security/getdatamaskednameapplicantsendotp',{
      ...
      if(data.type === 'success'){
        if(this.forward == false) {
          ...
          const text= 'SMS enviado nuevamente';
          this.toastService.show({text: text, type: 'success'});
        }
      }
      if(data.type === 'error'){
        this.phone = '88**-**88';
